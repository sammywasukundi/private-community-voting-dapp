...
const errorMessage = (e: unknown): string => {
  const causeSuffix =
    e instanceof Error && e.cause ? ` — cause : ${String(e.cause)}` : "";
...
const VotingPanel: React.FC<Props> = ({ providers, requestedAddress }) => {
    ...
    try {
      ...
      try {
        ...
        localStorage.setItem(
          STORAGE_PREFIX + address,
          btoa(String.fromCharCode(...secretKey)),
        );
  ...
  const handleJoin = async () => {
    if (!providers || !addressInput.trim()) return;
    ...
    try {
      ...
      if (!state) {
        push("error", t("error.contractNotFound"));
        return;
      }
      setResults(null);
      setContractAddress(address);
  ...
  const handleVote = async (choice: boolean) => {
    ...
    try {
      ...
      push(
        "success",
        t("poll.voteRegistered", {
          choice: choice ? t("poll.yes") : t("poll.no"),
        }),
      );
    } catch (e) {
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <button
                  className="ghost"
                  onClick={async () => {
                    const tx = sessionStorage.getItem(
                      "midnight:lastSubmittedTxId",
                    );
                    if (tx) {
        ...
        <>
          <div className="results">
            {results ? (
              <>
                <div className="result-item">
                  <div className="value" key={`status-${results.status}`}>
                    {results.status === PollStatus.CLOSED
                      ? t("poll.statusClosed")
                      : t("poll.statusOpen")}
                  </div>
                ...
                <div className="result-item">
                  <div className="value" key={`total-${results.total}`}>
                    {results.total.toString()}
                  </div>
                  <div className="label">{t("poll.total")}</div>
                </div>
                <div className="result-item">
                  <div className="value" key={`yes-${results.yes}`}>
                    {results.yes.toString()}
                  </div>
                  <div className="label">{t("poll.yes")}</div>
                </div>
                <div className="result-item">
                  <div className="value" key={`no-${results.no}`}>
                    {results.no.toString()}
                  </div>
              ...
              [0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="result-item skeleton"
                  style={{ height: 72 }}
                />
              ))
            )}
          ...
          {hasVoted && (
            <div className="already-voted-banner">
              <ShieldCheck size={16} />
              <div>
                <div className="already-voted-title">
                  {t("poll.alreadyVoted")}
                </div>
                <div className="already-voted-hint">
                  {t("poll.alreadyVotedHint")}
                </div>
              </div>
            </div>
          )}
